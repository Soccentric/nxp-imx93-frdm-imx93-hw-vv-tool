...
GPUTester::GPUTester() : gpu_available_(false) {
    ...
    gpu_available_ = fs::exists("/dev/dri/card0") || 
                     fs::exists("/dev/galcore") ||
...
TestReport GPUTester::short_test() {
    ...
    if (!gpu_available_) {
        return create_report(TestResult::NOT_SUPPORTED, "GPU not available or not accessible", std::chrono::milliseconds(0));
    }
    ...
    details << "\n";
    if (opengl_result != TestResult::SUCCESS && opengl_result != TestResult::NOT_SUPPORTED) all_passed = false;
    ...
    details << "\n";
    if (vulkan_result != TestResult::SUCCESS && vulkan_result != TestResult::NOT_SUPPORTED) all_passed = false;

    // Test GPU memory
    TestResult memory_result = test_gpu_memory();
    details << "GPU Memory: " << (memory_result == TestResult::SUCCESS ? "PASS" : "FAIL") << "\n";
    if (memory_result != TestResult::SUCCESS) all_passed = false;
...
TestReport GPUTester::monitor_test(std::chrono::seconds duration) {
    ...
    std::string details = "GPU monitoring completed for " + std::to_string(duration.count()) + " seconds";
...
TestResult GPUTester::test_opengl() {
    ...
    int result = system("glxgears -display :0 > /dev/null 2>&1 & sleep 1 && kill %1 > /dev/null 2>&1");
