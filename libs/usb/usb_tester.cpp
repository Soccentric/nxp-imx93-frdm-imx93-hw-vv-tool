...
TestReport USBTester::short_test() {
    ...
    if (!usb_available_) {
        return create_report(TestResult::NOT_SUPPORTED, "USB controllers not available", std::chrono::milliseconds(0));
    }
    ...
    for (const auto& controller : controllers_) {
        ...
        switch (controller.max_version) {
            case USBVersion::USB1_0: details << "USB 1.0"; break;
            case USBVersion::USB1_1: details << "USB 1.1"; break;
            case USBVersion::USB2_0: details << "USB 2.0"; break;
            case USBVersion::USB3_0: details << "USB 3.0"; break;
            case USBVersion::USB3_1: details << "USB 3.1"; break;
            case USBVersion::USB3_2: details << "USB 3.2"; break;
        }
    ...
    for (const auto& device : devices_) {
        if (device.connected) {
            details << "- " << device.product_name << " (" << device.vendor_id << ":" << device.product_id;
            if (device.high_speed) details << ", High Speed";
            if (device.super_speed) details << ", Super Speed";
            details << ")\n";
        }
    }
    ...
    if (controller_result != TestResult::SUCCESS) all_passed = false;
    ...
    if (device_result != TestResult::SUCCESS) all_passed = false;

    // Test USB transfer
    TestResult transfer_result = test_usb_transfer();
    details << "USB Transfer: " << (transfer_result == TestResult::SUCCESS ? "PASS" : (transfer_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (transfer_result != TestResult::SUCCESS && transfer_result != TestResult::NOT_SUPPORTED) all_passed = false;

    // Test USB power
    TestResult power_result = test_usb_power();
    details << "USB Power: " << (power_result == TestResult::SUCCESS ? "PASS" : "FAIL") << "\n";
    if (power_result != TestResult::SUCCESS) all_passed = false;
...
TestReport USBTester::monitor_test(std::chrono::seconds duration) {
    ...
    std::string details = "USB monitoring completed for " + std::to_string(duration.count()) + " seconds";
...
USBDeviceInfo USBTester::parse_usb_device_info(const std::string& device_path) {
    ...
    if (speed_file.is_open()) {
        ...
        } else if (speed.find("5000") != std::string::npos || speed.find("10000") != std::string::npos) {
