...
NetworkingTester::NetworkingTester() : networking_available_(false) {
    ...
    networking_available_ = fs::exists("/proc/net/dev") ||
                           system("which ip > /dev/null 2>&1") == 0;
...
TestReport NetworkingTester::short_test() {
    ...
    if (!networking_available_) {
        return create_report(TestResult::NOT_SUPPORTED, "Networking not available", std::chrono::milliseconds(0));
    }
    ...
    details << "Connectivity: " << (connectivity_result == TestResult::SUCCESS ? "PASS" : (connectivity_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (connectivity_result != TestResult::SUCCESS && connectivity_result != TestResult::NOT_SUPPORTED) all_passed = false;

    // Test DNS resolution
    TestResult dns_result = test_dns_resolution();
    details << "DNS Resolution: " << (dns_result == TestResult::SUCCESS ? "PASS" : "FAIL") << "\n";
    if (dns_result != TestResult::SUCCESS) all_passed = false;

    // Test latency
    TestResult latency_result = test_latency();
    details << "Latency: " << (latency_result == TestResult::SUCCESS ? "PASS" : (latency_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (latency_result != TestResult::SUCCESS && latency_result != TestResult::NOT_SUPPORTED) all_passed = false;
...
TestReport NetworkingTester::monitor_test(std::chrono::seconds duration) {
    ...
    std::string details = "Network monitoring completed for " + std::to_string(duration.count()) + " seconds";
...
std::string NetworkingTester::get_default_gateway() {
    ...
    while (std::getline(route_file, line)) {
        std::istringstream iss(line);
        std::string interface, destination, gateway, flags, refcnt, use, metric, mask, mtu, window, irtt;

        if (iss >> interface >> destination >> gateway >> flags >> refcnt >> use >> metric >> mask >> mtu >> window >> irtt) {
