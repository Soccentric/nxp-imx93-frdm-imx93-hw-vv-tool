...
TestReport CameraTester::short_test() {
    ...
    if (!camera_available_) {
        return create_report(TestResult::NOT_SUPPORTED, "Camera interfaces not available", std::chrono::milliseconds(0));
    }
    ...
    details << "MIPI CSI-2: " << (csi2_result == TestResult::SUCCESS ? "PASS" : (csi2_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (csi2_result != TestResult::SUCCESS && csi2_result != TestResult::NOT_SUPPORTED) all_passed = false;

    // Test multi-camera support
    TestResult multi_result = test_multi_camera();
    details << "Multi-camera: " << (multi_result == TestResult::SUCCESS ? "PASS" : (multi_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (multi_result != TestResult::SUCCESS && multi_result != TestResult::NOT_SUPPORTED) all_passed = false;
...
TestReport CameraTester::monitor_test(std::chrono::seconds duration) {
    ...
    std::string details = "Camera monitoring completed for " + std::to_string(duration.count()) + " seconds";
