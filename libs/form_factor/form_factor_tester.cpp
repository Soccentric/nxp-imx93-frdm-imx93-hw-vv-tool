...
FormFactorTester::FormFactorTester() : form_factor_available_(false) {
  ...
  form_factor_available_ = fs::exists("/proc/device-tree") ||
                           fs::exists("/sys/firmware/devicetree") || fs::exists("/sys/class/gpio");
...
TestReport FormFactorTester::short_test() {
  ...
  if (!form_factor_available_) {
    return create_report(TestResult::NOT_SUPPORTED, "Form factor testing not available",
  ...
  if (board_result != TestResult::SUCCESS)
    all_passed = false;

  // Test GPIO pins
  TestResult gpio_result = test_gpio_pins();
  details << "GPIO Pins: "
          << (gpio_result == TestResult::SUCCESS
                  ? "PASS"
                  : (gpio_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL"))
          << "\n";
  if (gpio_result != TestResult::SUCCESS && gpio_result != TestResult::NOT_SUPPORTED)
    all_passed = false;
  ...
  details << "Temperature: "
          << (temp_result == TestResult::SUCCESS
                  ? "PASS"
                  : (temp_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL"))
          << "\n";
...
TestReport FormFactorTester::monitor_test(std::chrono::seconds duration) {
  ...
  std::string details =
      "Interface monitoring completed for " + std::to_string(duration.count()) + " seconds";
