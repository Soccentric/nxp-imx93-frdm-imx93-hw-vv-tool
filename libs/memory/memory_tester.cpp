...
TestReport MemoryTester::short_test() {
    ...
    if (!memory_available_) {
        return create_report(TestResult::NOT_SUPPORTED, "Memory information not available", std::chrono::milliseconds(0));
    }
    ...
    if (integrity_result != TestResult::SUCCESS) all_passed = false;
    ...
    if (bandwidth_result != TestResult::SUCCESS) all_passed = false;

    // Test ECC if supported
    TestResult ecc_result = test_ecc();
    details << "ECC Test: " << (ecc_result == TestResult::SUCCESS ? "PASS" : (ecc_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (ecc_result != TestResult::SUCCESS && ecc_result != TestResult::NOT_SUPPORTED) all_passed = false;
...
TestReport MemoryTester::monitor_test(std::chrono::seconds duration) {
    ...
    std::string details = "Memory monitoring completed for " + std::to_string(duration.count()) + " seconds";
...
MemoryInfo MemoryTester::get_memory_info() {
    ...
    FILE* dmidecode_pipe = popen("dmidecode -t memory 2>/dev/null | grep -A 10 'Memory Device' | grep 'Type:' | head -1", "r");
    if (dmidecode_pipe) {
        char buffer[128];
...
TestResult MemoryTester::test_ram_integrity() {
    ...
    if (!std::all_of(test_buffer.begin(), test_buffer.end(), [](uint8_t val) { return val == 255; })) {
        return TestResult::FAILURE;
    }
...
TestResult MemoryTester::monitor_memory_usage(std::chrono::seconds duration) {
    ...
    return (usage_variation <= 0.1) ? TestResult::SUCCESS : TestResult::FAILURE; // Allow 10% variation
}
...
TestResult MemoryTester::stress_test_memory(size_t test_size_mb) {
    ...
    if (test_size_bytes > memory_info_.available_ram_mb * 1024 * 1024 * 0.8) { // Use 80% of available
        return TestResult::NOT_SUPPORTED;
    }
