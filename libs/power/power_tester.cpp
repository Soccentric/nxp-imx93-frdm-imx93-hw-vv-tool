...
PowerTester::PowerTester() : power_available_(false) {
    ...
    power_available_ = fs::exists("/sys/class/power_supply") ||
                      fs::exists("/sys/class/regulator") ||
...
TestReport PowerTester::short_test() {
    ...
    if (!power_available_) {
        return create_report(TestResult::NOT_SUPPORTED, "Power management not available", std::chrono::milliseconds(0));
    }
    ...
    switch (power_info_.source) {
        case PowerSource::BATTERY: details << "Battery"; break;
        case PowerSource::AC_ADAPTER: details << "AC Adapter"; break;
        case PowerSource::USB_C: details << "USB-C"; break;
        case PowerSource::POE: details << "PoE"; break;
        case PowerSource::UNKNOWN: details << "Unknown"; break;
    }
    details << "\n";
    ...
    if (source_result != TestResult::SUCCESS) all_passed = false;

    // Test power monitoring
    TestResult monitor_result = test_power_monitoring();
    details << "Power Monitoring: " << (monitor_result == TestResult::SUCCESS ? "PASS" : (monitor_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (monitor_result != TestResult::SUCCESS && monitor_result != TestResult::NOT_SUPPORTED) all_passed = false;

    // Test battery if present
    TestResult battery_result = test_battery();
    details << "Battery Test: " << (battery_result == TestResult::SUCCESS ? "PASS" : (battery_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL")) << "\n";
    if (battery_result != TestResult::SUCCESS && battery_result != TestResult::NOT_SUPPORTED) all_passed = false;

    // Test power management
    TestResult pm_result = test_power_management();
    details << "Power Management: " << (pm_result == TestResult::SUCCESS ? "PASS" : "FAIL") << "\n";
    if (pm_result != TestResult::SUCCESS) all_passed = false;
...
TestReport PowerTester::monitor_test(std::chrono::seconds duration) {
    ...
    std::string details = "Power monitoring completed for " + std::to_string(duration.count()) + " seconds";
    ...
    if (info.source == PowerSource::UNKNOWN) {
        ...
        if (fs::exists("/sys/class/power_supply/AC") || fs::exists("/sys/class/power_supply/ACAD")) {
            std::string ac_path = fs::exists("/sys/class/power_supply/AC") ?
                                 "/sys/class/power_supply/AC" : "/sys/class/power_supply/ACAD";
