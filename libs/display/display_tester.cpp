...
TestReport DisplayTester::short_test() {
  ...
  if (!display_available_) {
    return create_report(TestResult::NOT_SUPPORTED, "Display interfaces not available",
  ...
  details << "HDMI: "
          << (hdmi_result == TestResult::SUCCESS
                  ? "PASS"
                  : (hdmi_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL"))
          << "\n";
  if (hdmi_result != TestResult::SUCCESS && hdmi_result != TestResult::NOT_SUPPORTED)
    all_passed = false;

  // Test MIPI DSI functionality
  TestResult dsi_result = test_mipi_dsi();
  details << "MIPI DSI: "
          << (dsi_result == TestResult::SUCCESS
                  ? "PASS"
                  : (dsi_result == TestResult::NOT_SUPPORTED ? "N/A" : "FAIL"))
          << "\n";
...
TestReport DisplayTester::monitor_test(std::chrono::seconds duration) {
  ...
  std::string details =
      "Display monitoring completed for " + std::to_string(duration.count()) + " seconds";
